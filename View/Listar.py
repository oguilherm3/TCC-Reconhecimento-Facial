...
from PyQt5.QtWidgets import QTableView, QHeaderView, QTableWidgetItem, QPushButton, QItemDelegate, QMessageBox, QWidget
import os

class Ui_MainWindow(object):
    logo_path = os.getcwd().replace('View', 'Resources').replace('\\', '/') + '/Images/unip-logo.png'
    alunos = Aluno.get_lista(Aluno('', '', '', ''))
    ...
    def monta_lista(self, tabela):
        ...
        for row_idx, row_data in enumerate(self.alunos):
            ...
            item_course = QTableWidgetItem(str(row_data['course']))
            item_campus = QTableWidgetItem(str(row_data['campus']))
            item_phone = QTableWidgetItem(str(row_data['phone']))
            ...
            tabela.setItem(row_idx, 1, item_course)
            tabela.setItem(row_idx, 2, item_campus)
            tabela.setItem(row_idx, 3, item_phone)
    ...
    def atualiza_Aluno(self):
        ...
        aluno['curso'] = self.tableWidget.item(idx, 1).text()
        aluno['campus'] = self.tableWidget.item(idx, 2).text()
        aluno['phone'] = self.tableWidget.item(idx, 3).text()
